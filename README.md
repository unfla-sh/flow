...
- **Diagram kits** — switch the palette, connection defaults, validation, and tools between Workflow, Organisation, Database ER, Infrastructure, Image Generation, State machine, Planning, UML class, and General diagrams. Starter templates are included for each specialized kit.
- **Twenty diagram types, all editable** — File ▸ New from template is grouped by Structure (architecture, IT current-state, flowchart, state machine, swimlane), Hierarchy (nested, tree, layer stack, UML class, story map), Flow & process (loop, high-level, process, data flow, kanban, deployment, dependency graph, database schema), and Data architecture (medallion, DP integration). Each is plain nodes, frames, and edges, so anything can be dragged, restyled, or rewired.
- **UML and schema detail** — record cards carry an operations section (methods) and per-column index badges; edges can end in open triangles, filled or open diamonds, or circles for inheritance, composition, and aggregation.
...
- **Generate with AI** — describe any supported diagram, copy the generated prompt into **any** chat assistant (Poe, ChatGPT, Claude.ai, Gemini…), and paste its JSON answer back. The prompt includes exact recipes and component ids for workflow, organization, ER, infrastructure, image-generation, and tournament-style templates. No API key, provider config, or server submission is required — by design, since the editor is a public static site.
- **Modify with AI** — the **AI** toolbar button (or File → Modify with AI…) builds a prompt from the current diagram plus a change request ("add a manager-approval step after review", "colour the database nodes blue"); paste the answer back and it is applied in place. New nodes are placed next to the nodes they connect to; the whole change is one undo step.
- **Adjustable arrows** — select an edge and drag the small handle on the line (or double-click the line) to add a bend the line actually passes through; bends snap to straight segments (hold Alt to place freely), double-click removes one, and the Route tab lists them numerically. Drag either arrow end onto another node to re-attach it.
- **Templates** — File ▸ New from template is grouped: twenty diagram-type starters, the kit starters (Organisation, Database ER, Infrastructure, Image Generation), five feature tours, and two reference diagrams.
