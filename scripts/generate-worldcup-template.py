...
# the real 2026 World Cup knockout results (source: Wikipedia, as of Jul 9).
import json
...
# ─ Quarter-finals: real fixtures, not yet played (no scores, no bold).
match_node('qf_l1', 760, 210, 'Boston', 'Jul 9', ('FRA', None, False), ('MAR', None, False), 'QF')
match_node('qf_l2', 760, 770, 'Los Angeles', 'Jul 10', ('ESP', None, False), ('BEL', None, False), 'QF')
match_node('qf_r1', 3040, 210, 'Miami', 'Jul 11', ('NOR', None, False), ('ENG', None, False), 'QF')
match_node('qf_r2', 3040, 770, 'Kansas City', 'Jul 11', ('ARG', None, False), ('SUI', None, False), 'QF')

# ─ Semi-finals, final and 3rd place: participants still to be decided, so
#   each row carries the flags of the teams it can come from.
def flags(*codes):
    return ' '.join(T[c][0] for c in codes)

card_node('sf_l', 1140, 490, 'Dallas', 'Jul 14',
          [{'label': f"{flags('FRA', 'MAR')} winner"},
           {'label': f"{flags('ESP', 'BEL')} winner"}],
          'SF · Dallas')
card_node('sf_r', 2660, 490, 'Atlanta', 'Jul 15',
          [{'label': f"{flags('NOR', 'ENG')} winner"},
           {'label': f"{flags('ARG', 'SUI')} winner"}],
          'SF · Atlanta')
card_node('final', 1900, 450, 'New York', 'Jul 19',
          [{'label': f"{flags('FRA', 'MAR', 'ESP', 'BEL')} winner"},
           {'label': f"{flags('NOR', 'ENG', 'ARG', 'SUI')} winner"}],
          'Final · New York', emblem='🏆', accent=True)
card_node('third', 1900, 700, 'Miami', 'Jul 18',
          [{'label': f"{flags('FRA', 'MAR', 'ESP', 'BEL')} loser"},
           {'label': f"{flags('NOR', 'ENG', 'ARG', 'SUI')} loser"}],
...
doc = {
    ...
    'settings': {
        ...
        'version': '1.2.0',
        'description': 'Mirrored 32-team knockout bracket built from score-card nodes: venue header, flag/name/score rows, winner in bold, gold final with trophy and a 3rd-place match. Real results through the Round of 16; quarter-finals onward are upcoming fixtures.',
    },
    'flows': {
        'root': {
 ...
 * Real results through the Round of 16 (per Wikipedia, as of Jul 9 2026);
 * quarter-finals onward are the real upcoming fixtures with dates as tags.
